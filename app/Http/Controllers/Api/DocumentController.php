...
class DocumentController extends Controller
{
    ...
    public function index(DocumentRequest $Request)
    {
        ...
        // From and to items processed
        $this->response['from'] = $processor->start();
        $this->response['to'] = $processor->end();
        // First and last items in document
        $this->response['first'] = $processor->first();
        $this->response['last'] = $processor->last();
        // Get total number or records
        $this->response['total'] = $processor->total();
        $this->response['count'] = $processor->count();
        // Get processed data size (bytes)
        $this->response['size'] = $processor->size();
        // Document hash
        $this->response['hash'] = $document->hash;
        // Document type
        $this->response['type'] = $document->type;
     ...
     * V2 Route
     */
    public function indexV2(DocumentRequest $Request)
    {
        // Validate request
        $request = $Request->validated();
        // Get data
        ...
        return [
            'data' => $data ?? [],
            'type' => 'document',
            'id' => $this->response['hash'],
            'attributes' => [
                'document_url' => $request['url'],
                'document_type' => $this->response['type'],
                ...
                'data_count' => $this->response['count'],
                'data_from' => $this->response['from'],
                'data_to' => $this->response['to'],
                'data_next' => $this->next() ?? null,
                'data_first' => $this->response['first'],
                'data_last' => $this->response['last'],
                'data_total' => $this->response['total'],
                'data_size' => $this->response['size'],
    ...
    {
        $total = $this->response['total']+1;
        $next = $this->response['to']+1;
